...
class Gui018{
	...
	public Gui018() {
		...
		button = new JButton[] {new JButton("ONE"),new JButton("TWO"),new JButton("THREE"),new JButton("FOUR"),new JButton("FIVE"),new JButton("SIX")};
		panel = new JPanel();
	} // end constructor
...
//		frame.add(button[5], BorderLayout.CENTER);
		frame.setSize(600,500);
		frame.setVisible(true);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.setLocationRelativeTo(null); // 가운데 팝업
		panel.setLayout(new GridLayout(2,1)); // 순서를 버튼[4][5] 위로 올린다
		panel.add(button[4]);
		panel.add(button[5]);
	}	
}
public class Gui018_BorderLayout_GridLayout {
