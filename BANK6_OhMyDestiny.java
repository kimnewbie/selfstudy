	...
	public static void main(String[] args) {
		...
		for(;;) {
			...
			case 3 : {System.out.println("ㅁ 입금기능입니다.");
				//변수
				String temp_id="", temp_pass=""; int temp_balance=0; int find = -1; 
				//입력
				System.out.print("아이디 : "); temp_id = scanner.next();
				System.out.print("비밀번호 : "); temp_pass = scanner.next();
				//처리(1) - 유저확인
				...
				//처리(2) - 입금처리
				if(find!=-1) {
					System.out.print("입금할 금액 > "); temp_balance = scanner.nextInt();
					if(temp_balance>0) {
						users[find][2] += Integer.parseInt(users[find][2]) + temp_balance;
						System.out.println("ㅁ 아이디 : " + users[find][0] + "\n비밀번호 : " + users[find][1]
								+ "\n잔액 : " + users[find][2]);
			...
			case 4 : {System.out.println("ㅁ 출금기능입니다.");
				...
				if(find!=-1) {
					...
					if(temp_balance<Integer.parseInt(users[find][2])) {
						users[find][2] += Integer.parseInt(users[find][2]) - temp_balance;
						System.out.println("ㅁ 아이디 : " + users[find][0] + "\n비밀번호 : " + users[find][1]
								+ "\n잔액 : " + users[find][2]);
